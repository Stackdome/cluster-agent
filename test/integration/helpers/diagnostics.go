...
func DumpBuildDiagnostics(ctx context.Context, c client.Client, kubeClient kubernetes.Interface, namespace string) string {
	...
	ds := &appsv1.DaemonSet{}
	dsKey := client.ObjectKey{Name: "registry-config-reconciler", Namespace: bootstrap.RegistryNamespace}
	if err := c.Get(ctx, dsKey, ds); err != nil {
		...
		if err := c.List(ctx, dsPods, client.InNamespace(bootstrap.RegistryNamespace), client.MatchingLabels{"daemonset-for": "registry-config-reconciler"}); err == nil {
			for _, p := range dsPods.Items {
				...
				out += fetchPodLogs(ctx, kubeClient, bootstrap.RegistryNamespace, p.Name, 20)
			}
		}
	}
